h24408
s 00076/00000/00000
d D 1.1 01/07/06 11:42:20 greischar 1 0
c date and time created 01/07/06 11:42:20 by greischar
e
u
U
f e 0
...
case 'newFeatures',
   ...
   helpText(1,10) = {'     '};
	helpText(1,11) = {'      Vertical scales are now located on alternating ends of the plots since they all could have different scales and need to be labelled. Selecting data with the left button marks it bad, with the right button good; or you can change your mind in the Edit Options window as before.  Click on the numGoodWins button to compute the total number of good 1sec/0.5sec overlap windows in the scored data. This needs to be manually updated after additional scoring since it can take about 20s on 128 channel data.'}; 


case 'chanSettings',
	helpText(1,1) = {'     Modifications to the channel display (and also to the config file) are made using a new button called ''chan settings'' where you can edit each channel''s name, display order, rangeoffset, and set whether to use the rangeoffset values, whether the channel will be displayed (visibility), and whether the channel should be displayed in a fixed window at the bottom of the screen.'};
...
case 'configFile',
	helpText(1,1) = {'     eegscoreNEW requires a more complicated configuration file than the previous version. After you hit ''load'' it will prompt you whether to create a default, open an existing, or modify a configuration file.  Choose create default if this is the first time scoring this type of data.  A basic configuration file will be created which tells eegscoreNEW to display the channels in numerical order with channels named with their display number, and no fixed channels or channel with adjusted range or bias. If you have chosen create a new or modify and existing configuration file then at the end of the scoring session you will be prompted to save the configuration file (and change its name) with all the modifications made during the session.'};
