function convertHR3D()
%%% load IBI files, convert them to HR and arange in a data set. 
homeDir = GetASBasePath();

SHOW_FIGURES=0;
data_dir=[homeDir, '\Matlab\data\PreProcessedData\ECG_3D\Excell\all\'];
% output_dir=[homeDir, '\Matlab\data'];
output_dir=[homeDir, '\Matlab\data\ProcessedData\HR_3D\'];
...
%     end
    ...
    HRdata(ind).HR=HR(:);
        if SHOW_FIGURES,
            ...
            plot(HRdata(ind).HR);
            title(files(i).name)
            savepath=[homeDir '\Matlab\data\PreProcessedData\ECG_3D\figures\'];
