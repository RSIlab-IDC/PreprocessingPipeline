...
SAMPLE_RATE = 1000;
DOWNSAMPLE_RATE=100;
RAW_DOWNSAMPLE_RATE=100;
% the data structure with all the experiment data
datafileName = [GetASBasePath(), '\Matlab\data\ASRawData3D.mat'];
...
expFilesFolder = [GetASBasePath(), '\rawdata\acq_3D'];
...
for i=3:length(files)
    ...
    subject_ind = length(Subjects_Id)+1;
    exist=0;
    ...
    if exist
        if isempty(ASData(subject_ind).(taskName))
        ...
        ASData(subject_ind).SubjectID = Subject_Id;
...
if badListExist
    badSignals = loadBadDataList3D(ASRawData);
else
    badSignals = {};
end
taskChannels = {};
...
ASData = getASDataFromRaw(ASRawData, badSignals, tasks, taskChannels);
ASData = loadRatingFiles3D (ASData, badSignals);
ASRawData = loadRatingFiles3D (ASRawData, badSignals);
ASRawData = removeProcesedSignals(ASRawData, tasks);
% saving the data structure to a file
dirToSave = [GetASBasePath(),'\Matlab\data\ASRawData3D.mat'];
save(dirToSave, 'ASRawData' , '-v7.3') ;
% saving the data structure to a file
dirToSave = [GetASBasePath(),'\Matlab\data\ASData3D'];
