...
if ~isempty(outRawData)
    ...
    LENDATA = length(outData.EMG_POWER_Corr);
 
    outData.ECG = outRawData(:,ECG_1);
    outData.RSP = outRawData(:,RSP_1);
    
    temp = downsample(outRawData(:,HR_1),dSample);
    outData.HR = temp(1:LENDATA);
    
    temp = downsample(outRawData(:,RMS_Zyg_1),dSample);
    outData.RMS_Zyg = temp(1:LENDATA);
    
    temp = downsample(outRawData(:,RMS_Corr_1),dSample);
    outData.RMS_Corr = temp(1:LENDATA);
    
    temp = downsample(outRawData(:,RMS_Orb_1),dSample);
    outData.RMS_Orb = temp(1:LENDATA);
